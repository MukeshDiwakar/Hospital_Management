...
void LoginScreen(void)		//function for login screen
{
	...
	char Username[15];
	char Password[15];
	char original_Username[25]="mukesh";
	char original_Password[15]="4554";
	do
	{
...
void Add_rec(void)
{
   ...
   /* **********************Age ********************** */
    ...
    do
    {
	    ...
	    if(strlen(p.Address)>50||strlen(p.Address)<4)
		{
			printf("\n\t Invalid :( \t The max range for address is 50 and min range is 4 :)");
			goto C;
		}
	}while(!valid);
	/* *********************** Contact no. ************************** */
